...
if trend>0:
    print("Weak High" ,dfLB.time[trail_up_x], enddate,trail_up,trail_dn)
else:
    print("Strong High " , dfLB.time[trail_up_x], enddate,trail_up,trail_dn)
i=7
eq_prv_top=0
eq_top_x = 0
eq_prv_btm = 0.
...
for i in range(0,len(target_btm)-1):
    if target_type[i]==-1:
        print("RED type ",dfLB.time[target_left[i]],target_btm[i],target_top[i])
    elif target_type[i]==1:
        print("GREY type ",dfLB.time[target_left[i]],target_btm[i],target_top[i])